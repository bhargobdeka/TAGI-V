...
%             idxobs      = randperm(numObs);
            ...
            xtrain      = x(idxTrain, :);
            ytrain      = y(idxTrain, :);
%             idxTr       = randperm(length(xtrain));
%             xtrain      = xtrain(idxTr,:);
%             ytrain      = ytrain(idxTr,:);
            xtest       = x(idxTest, :);
            ytest       = y(idxTest, :);
%             idxTe       = randperm(length(xtest));
%             xtest       = xtest(idxTe,:);
%             ytest       = ytest(idxTe,:);
        end
        ...
        function [xtrain, ytrain, xval, yval,trainIdx,valIdx ] = partition(x, y, c , n)
            trainIdx = training(c,n);
            xtrain   = x(trainIdx,:);
            ytrain   = y(trainIdx,:);
            valIdx   = test(c,n);
            xval     = x(valIdx,:);
        ...
        function [xtrain, ytrain, xval, yval] = holdout(x, y)
            n          = length(x);
            hpartition = cvpartition(n,'Holdout',0.2); % Nonstratified partition
