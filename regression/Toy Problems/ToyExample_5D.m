...
ntrain      = 400;
ntest       = 100;
f           = @(x1,x2,x3,x4,x5) (3*(x1+x2+x3+x4+x5).^2)/50;
noise_het   = @(x1,x2,x3,x4,x5) (0.1*(x1+x2+x3+x4+x5).^2);      %(3*(x).^4)+0.02, (3*(x).^4)+0.01
noise_hom   = 0.5;
xtrain1     = unifrnd(-1,1,[ntrain,1]);
xtrain2     = unifrnd(-1,1,[ntrain,1]);
xtrain3     = unifrnd(-1,1,[ntrain,1]);
xtrain4     = unifrnd(-1,1,[ntrain,1]);
xtrain5     = unifrnd(-1,1,[ntrain,1]);

xtrain      = [xtrain1 xtrain2 xtrain3 xtrain4 xtrain5];

xtest1      = linspace(-1,1,ntest)';
xtest2      = linspace(-1,1,ntest)';
xtest3      = linspace(-1,1,ntest)';
xtest4      = linspace(-1,1,ntest)';
xtest5      = linspace(-1,1,ntest)';
xtest       = [xtest1 xtest2 xtest3 xtest4 xtest5];

noiseTrain  = noise_het(xtrain1,xtrain2,xtrain3,xtrain4,xtrain5)+noise_hom;

ytrainTrue  = f(xtrain1,xtrain2,xtrain3,xtrain4,xtrain5);
ytrain      = f(xtrain1,xtrain2,xtrain3,xtrain4,xtrain5) + normrnd(zeros(length(noiseTrain), 1), sqrt(noiseTrain));

noiseTest  = noise_het(xtest1,xtest2,xtest3,xtest4,xtest5)+noise_hom;
ytestTrue  = f(xtest1,xtest2,xtest3,xtest4,xtest5);
